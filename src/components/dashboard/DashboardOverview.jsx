    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-5">
            {isViewer ? (
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-teal-500 hover:bg-teal-400 text-white rounded-xl text-xs font-extrabold shadow-lg shadow-teal-500/30 transition transform hover:-translate-y-0.5"
              >
                <ShieldCheck className="w-4 h-4" />
                Jelajahi Dokumen Aktif (Controlled Copy)
              </button>
            ) : (
              <>
                {canRegisterDocument && (
      ...
      {isViewer ? (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div
            onClick={() => navigateTo('ctrl-active', ['Dashboard', 'Document Control', 'Dokumen Aktif'])}
            className="bg-white dark:bg-slate-900 rounded-xl shadow-card border border-slate-200 dark:border-slate-800 p-5 cursor-pointer hover:border-teal-400 hover:shadow-md transition group"
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-teal-600 uppercase tracking-wider">Dokumen Aktif Terkendali</span>
              <ShieldCheck className="w-5 h-5 text-teal-600 group-hover:scale-110 transition" />
            </div>
            <div className="text-3xl font-extrabold text-teal-700 dark:text-teal-400 mt-3 font-mono">
              {activeDocs}
            </div>
            <div className="text-xs text-slate-500 mt-1.5 flex items-center gap-1 font-medium">
              <span>Klik untuk telusuri SOP & Instruksi Kerja</span>
              <ArrowUpRight className="w-3.5 h-3.5 text-teal-500" />
            </div>
          </div>

          {/* Departemen Tersedia */}
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-card border border-slate-200 dark:border-slate-800 p-5 group">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-blue-600 uppercase tracking-wider">Departemen Terdaftar</span>
            ...
            <div className="text-xs text-slate-400 mt-1.5 font-medium">
              Cakupan seluruh divisi operasional PT DJI
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-card border border-slate-200 dark:border-slate-800 p-5 group">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-purple-600 uppercase tracking-wider">Kategori Standar ISO</span>
              <Layers className="w-5 h-5 text-purple-600 group-hover:scale-110 transition" />
            </div>
            <div className="text-3xl font-extrabold text-slate-900 dark:text-white mt-3 font-mono">
              {documentTypes.length}
            </div>
            <div className="text-xs text-slate-400 mt-1.5 font-medium">
              SOP, Instruksi Kerja, Form & Kebijakan
            </div>
          </div>
        </div>
      ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {/* Quick Pending Verification Widget (Hidden for Viewer) */}
          {!isViewer && (
